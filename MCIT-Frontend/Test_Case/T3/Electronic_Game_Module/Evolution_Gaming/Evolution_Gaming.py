...
class ElectronicGameEVOModule(Base_Setup.BSetup):

    def test_TC_EVO_01_Navigate_to_EvolutionGaming_BeforeLogin(self):
        print("<b> Expected Results: Login Reminder modal dialog is displayed when accessing Evolution Gaming game without valid login. </b>" + "<br>")
        MainpageActions.Access_to_Mainpage(self)
        ElectronicGamepageActions.Access_to_ElectronicGame_Tab(self)
        ElectronicGamepageActions.Access_to_EvolutionGaming_Page(self)
        game_position = self.driver.find_element_by_xpath(ElectronicGameElement.evo_game)
        actions = ActionChains(self.driver)
        actions.move_to_element(game_position).perform()
        print("<li>" + "Click on an Evolution Gaming game in the list: " + str(ElectronicGameData.loopranevo) + "</li>" + "<br>")
        wait_page_load = EC.element_to_be_clickable((By.XPATH, ElectronicGameElement.evo_game))
        WebDriverWait(self.driver, 10).until(wait_page_load)
        select_game = self.driver.find_element_by_xpath(ElectronicGameElement.evo_game).click()
        ElectronicGamepageActions.Assert_Login_Reminder(self)


    def test_TC_EVO_02_Navigate_to_EvolutionGaming_AfterLogin(self):
    ...
    def test_TC_EVO_03_Enter_Game(self):
    ...
    def test_TC_EVO_04_Quick_Transfer(self):
