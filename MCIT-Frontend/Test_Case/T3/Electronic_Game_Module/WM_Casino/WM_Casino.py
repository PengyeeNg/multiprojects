...
class ElectronicGameWMModule(Base_Setup.BSetup):

    def test_TC_WM_01_Navigate_to_WMCasino_BeforeLogin(self):
        print("<b> Expected Results: Login Reminder modal dialog is displayed when accessing WM Casino game without valid login. </b>" + "<br>")
        ...
        game_position = self.driver.find_element_by_xpath(ElectronicGameElement.wmcasino_game)
        actions = ActionChains(self.driver)
        actions.move_to_element(game_position).perform()
        print("<li>" + "Click on a WM Casino game in the list: " + str(ElectronicGameData.loopranwm) + "</li>" + "<br>")
        wait_page_load = EC.element_to_be_clickable((By.XPATH, ElectronicGameElement.wmcasino_game))
        WebDriverWait(self.driver, 10).until(wait_page_load)
        select_game = self.driver.find_element_by_xpath(ElectronicGameElement.wmcasino_game).click()
    ...
    def test_TC_WM_03_Enter_Game(self):
    ...
    def test_TC_WM_04_Quick_Transfer(self):
