...
class ElectronicGameNextSpinModule(Base_Setup.BSetup):

    def test_TC_NextSpin_01_Navigate_to_NextSpin_BeforeLogin(self):
        print("<b> Expected Results: Login Reminder modal dialog is displayed when accessing NextSpin game without valid login. </b>" + "<br>")
        ...
        game_position = self.driver.find_element_by_xpath(ElectronicGameElement.nextspin_game)
        actions = ActionChains(self.driver)
        actions.move_to_element(game_position).perform()
        print("<li>" + "Click on a NextSpin game in the list: " + str(ElectronicGameData.looprannextspin) + "</li>" + "<br>")
        wait_page_load = EC.element_to_be_clickable((By.XPATH, ElectronicGameElement.nextspin_game))
        WebDriverWait(self.driver, 10).until(wait_page_load)
        select_game = self.driver.find_element_by_xpath(ElectronicGameElement.nextspin_game).click()
        ElectronicGamepageActions.Assert_Login_Reminder(self)


    def test_TC_NextSpin_02_Navigate_to_NextSpin_AfterLogin(self):
        print("<b> Expected Results: Quick Transfer modal dialog is displayed when accessing NextSpin game with valid login. </b>" + "<br>")
    ...
    def test_TC_NextSpin_03_Enter_Game(self):
