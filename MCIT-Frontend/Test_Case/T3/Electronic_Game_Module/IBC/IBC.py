...
class ElectronicGameIBCModule(Base_Setup.BSetup):
    ...
    def test_TC_IBC_01_Navigate_to_IBC_BeforeLogin(self):
        ...
        game_position = self.driver.find_element_by_xpath(ElectronicGameElement.ibc_game)
        actions = ActionChains(self.driver)
        actions.move_to_element(game_position).perform()
        print("<li>" + "Click on an IBC game in the list: " + str(ElectronicGameData.loopranibc) + "</li>" + "<br>")
        wait_page_load = EC.element_to_be_clickable((By.XPATH, ElectronicGameElement.ibc_game))
        WebDriverWait(self.driver, 10).until(wait_page_load)
        select_game = self.driver.find_element_by_xpath(ElectronicGameElement.ibc_game).click()
        ElectronicGamepageActions.Assert_Login_Reminder(self)


    def test_TC_IBC_02_Navigate_to_IBC_AfterLogin(self):
    ...
    def test_TC_IBC_03_Enter_Game(self):
